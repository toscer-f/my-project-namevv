...
public class AccesDataTest {
	ArrayList<TypeCharge> listeCharge;
	...
	// il faut donc que l'état initial au lancement de la méthode soit le même
	// que l'état final
	...
	public void testGetListeStation() {
		...
		// état initial 3 éléments dont 2 stations uniquement avec bornes(1 et
		// 2)
		...
		assertEquals(s.getLibelleEmplacement(), "GareMontParnasse");
	...
	public void testGetListeBorne() {
		...
		listeBorne = AccesData.getListBorne();
	...
	public void testGetListeBorneStation() {
		// station qui existe
		listeBorne = AccesData.getBorneStation(1);
		assertEquals(listeBorne.size(), 2);
		b = listeBorne.get(1);
		...
		listeBorne = AccesData.getBorneStation(4);
		assertEquals(listeBorne.size(), 0);
	...
	public void testAddDeleteTypeCharge() {
		...
		// suppresssion du dernier type charge inséré pour remettre la base en
		// état initial
		...
		assertTrue(AccesData.deleteTypeCharge(listeTypeCharge
				.get(listeTypeCharge.size() - 1)));
	}
	...
	public void testAddDeleteBorne() {
		...
		TypeCharge t = AccesData.getTypeCharge(1);
		Borne b = new Borne("12/12/2013", t, 1);
	...
	public void testAddDeleteStation() {
		...
		// récupération liste des stations , nouvelle station en position 0 dans
		// la liste
		ArrayList<Station> listeStation = AccesData.getListeStation();
		// suppressison de la station en première position dans la liste
		// remise en l'état initial avant fin méthode
		assertTrue(AccesData.DeleteStation(listeStation.get(0)));
	...
	public void testUpdateStation() {
		...
		// sauvegarde ancien libelle pour rétablissement situation en fin de
		// méthode
